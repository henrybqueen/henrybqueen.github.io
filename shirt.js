...
gl.vertexAttribPointer(0, 2, gl.FLOAT, false, 0, 0);
    ...
    float f(float x, float y){
        return square(cube(x)-3.0 * x * square(y) - 1.0) + square(3.0 * square(x) * y - cube(y));
    }
    ...
    void main() {
        ...
        p.x = scale *  (2.0  * (position.x / (N-1.0)) - 1.0);
        p.z = scale * (2.0  * (position.y / (M-1.0)) - 1.0);
...
void main() {
    ...
    outColor = vec4(vec3(atan(0.18 * z)), 1.0);
